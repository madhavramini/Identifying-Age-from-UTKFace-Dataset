...
from data_loader.UTKFace_datasets import UTKFaceData
...
def main(args):
    ...
    data_transforms = {
        'train': transforms.Compose([
            transforms.RandomResizedCrop(128), #reduced size to suit computational resources
            transforms.RandomHorizontalFlip(),
            transforms.ToTensor(),
            transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
        ]),
        'val': transforms.Compose([
            transforms.CenterCrop(128), #reduced size to suit computational resources
            transforms.ToTensor(),
            transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
        ])
    }

    train_datasets = UTKFaceData(os.path.join(args.data_root, 'train'), data_transforms['train'])
    val_datasets   = UTKFaceData(os.path.join(args.data_root, 'val'), data_transforms['val'])
    train_dataloaders = torch.utils.data.DataLoader(train_datasets, batch_size=16, shuffle=True, num_workers=6)
    val_dataloaders   = torch.utils.data.DataLoader(val_datasets, batch_size=32, shuffle=False, num_workers=6)
    ...
    if 'resnet18' == args.model.split('_')[0]:
        my_model = models.resnet18(weights=None)
    elif  'resnet50' == args.model.split('_')[0]:
        my_model = models.resnet50(weights=None)
    elif 'resnet50-cbam' == args.model.split('_')[0]:
        my_model = resnet_cbam.resnet50_cbam(weights=None)
    elif 'resnet101' == args.model.split('_')[0]:
        my_model = models.resnet101(weights=None)
    else:
    ...
    logger.append(f"Model: {args.model}")
    logger.append(f"Training samples: {len(train_datasets)}, Validation samples: {len(val_datasets)}")
    logger.append(f"Batch size: {args.batch_size}, Epochs: {num_epochs}")
